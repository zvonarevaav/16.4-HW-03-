...
from .tasks import send_news_after_post, send_week_news_at8am
...
def test_logging(request):
    logger.debug('Debug message from views.py')
    logger.info('Info message from views.py')
    logger.warning('Warning message from views.py')
    logger.error('Error message from views.py')
    logger.critical('Critical message from views.py')
    return HttpResponse("Check your logs")


@cache_page(60)  # кэширование на 1 минуту
def home(request):
    # ваш код для главной страницы
    return render(request, 'home.html')

@cache_page(300)  # кэширование на 5 минут
...
class CreateNews(PermissionRequiredMixin, CreateView):  # представление формы для создания новости
    ...
    def form_valid(self, form):
        ...
        send_news_after_post.delay(news.id)
        return response
...
class CreateArticle(PermissionRequiredMixin, CreateView):  # представление для создания статьи
    ...
    def form_valid(self, form):
        ...
        response = super().form_valid(form)
        send_news_after_post.delay(article.id)
        return response
